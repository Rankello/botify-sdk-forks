...
var convertHttpRequest = function (req) {
    ...
    if (req.formData) {
        var flattenedList = APIHelper.formDataEncodeObject(req.formData);
        for (var index in flattenedList) {
            var key = flattenedList[index].key
            var value = flattenedList[index].value;
